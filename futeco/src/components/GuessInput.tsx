    ...
    <div ref={containerRef} className="relative flex gap-3">
      <div className="relative flex-1">
        <input
          ...
          disabled={disabled}
          className="theme-input h-12 w-full rounded-md border px-4 text-sm outline-none shadow-sm focus:border-green-500 disabled:opacity-50"
        />

        {isOpen && filteredTeams.length > 0 && (
          <ul className="theme-surface absolute top-full z-10 mt-1 w-full overflow-hidden rounded-md border shadow-md">
            {filteredTeams.map((team) => (
              <li key={team.id}>
                <button
                  type="button"
                  onClick={() => handleSelect(team)}
                  className="theme-hover flex w-full items-center gap-2 px-3 py-2 text-left text-sm"
                >
